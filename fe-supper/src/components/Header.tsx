...
function DropdownItem({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left px-5 py-3 hover:bg-white/10 transition flex flex-col"
    >
      <span className="text-[11px] font-semibold text-white/90 uppercase tracking-wide">
        {label}
      </span>
      {desc && (
        <span className="text-[11px] text-white/65 leading-snug mt-0.5">
          {desc}
        </span>
      )}
    </button>
        ...
        <div className="bg-black/40 backdrop-blur-xl border-b border-white/10">
          <div className="mx-auto max-w-6xl px-4">
            ...
            <div className="flex h-16 items-center justify-between">
              ...
              {/* Desktop Nav */}
              <nav className="hidden lg:flex items-center gap-7 text-[13px] text-white/80">
                {NAV.map((group) => {
                  const hasDropdown = !!group.items?.length;
                  return (
                    <div key={group.label} className="relative group">
                      ...
                      {hasDropdown && (
                        <div className="absolute left-0 top-full pt-3 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:translate-y-0 group-focus-within:pointer-events-auto transition">
                          <div className="min-w-[260px] rounded-2xl bg-[#0B1218]/95 ring-1 ring-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden">
                            <div className="p-3 space-y-1">
                              {group.items!.map((it) => (
                                <a
                                  key={it.label}
                                  href={it.href}
                                  className="flex items-center justify-between rounded-xl px-4 py-2.5 text-[13px] text-white/85 hover:text-white hover:bg-white/10 transition"
                                >
                                  <span>{it.label}</span>
                                </a>
                              ))}
              ...
              <div className="flex items-center gap-3">
                ...
                {currentUser && (
                  <div className="hidden md:block relative" ref={userMenuRef}>
                    <button
                      onClick={() => setUserMenuOpen((v) => !v)}
                      className="inline-flex items-center gap-2 h-10 px-4 rounded-xl bg-white/5 ring-1 ring-white/10 hover:bg-white/10 text-[11px] font-medium transition"
                    >
                      <div className="h-8 w-8 rounded-full bg-emerald-400/20 ring-1 ring-emerald-400/60 grid place-items-center text-[11px] font-semibold text-emerald-200">
                        {initials}
                      </div>

                      <div className="flex flex-col items-start leading-tight">
                        <span className="text-[11px] font-semibold max-w-[160px] truncate">
                          {displayName}
                        </span>
                        <span className="text-[10px] tracking-wide text-emerald-300/90 uppercase">
                          {isAdmin ? "Admin" : "User"}
                        </span>
                      </div>
                    ...
                    {userMenuOpen && (
                      <div className="absolute right-0 mt-2 w-80 rounded-2xl bg-[#0A0F14] ring-1 ring-white/10 shadow-[0_18px_50px_rgba(0,0,0,0.75)] overflow-hidden text-xs animate-fadeIn">
                        {/* Header */}
                        <div className="px-5 py-4 border-b border-white/10 flex items-center gap-3">
                          <div className="h-9 w-9 rounded-full bg-emerald-400/20 ring-1 ring-emerald-400/50 grid place-items-center text-[12px] font-semibold text-emerald-200">
                            {initials}
                          </div>
                          <div className="flex-1">
                        ...
                        <div className="py-1">
                          ...
                          {isAdmin && (
                            <>
                              <div className="px-5 mt-3 mb-1 text-[10px] tracking-wide text-white/45 uppercase">
                                Administration
                              </div>
                        ...
                        <button
                          onClick={handleLogout}
                          className="w-full flex items-center justify-center gap-2 px-4 py-3 text-[12px] bg-red-500/90 text-black font-semibold hover:bg-red-400 transition"
                        >
                          <LogOut className="h-4 w-4" />
                          Logout
                        </button>
                ...
                {!currentUser && (
                  <>
                    <button
                      ...
                      className="hidden md:inline-flex items-center gap-1 h-9 px-3 rounded-xl bg-white/5 ring-1 ring-white/10 hover:bg-white/10 text-[12px] font-medium transition hover:-translate-y-[1px] active:translate-y-[1px] active:scale-[0.97]"
                    >
                      <LogIn className="h-3.5 w-3.5" />
                      Login
                    </button>
                    <button
                      ...
                      className="hidden md:inline-flex items-center gap-1 h-9 px-3 rounded-xl bg-emerald-400/10 ring-1 ring-emerald-400/60 text-[12px] font-medium text-emerald-300 hover:bg-emerald-400/20 transition hover:-translate-y-[1px] active:translate-y-[1px] active:scale-[0.97]"
                    >
                      <UserPlus className="h-3.5 w-3.5" />
                      Register
                    </button>
                      ...
                      return (
                        <div key={group.label} className="rounded-xl overflow-hidden">
                          <button
                            ...
                            className="w-full flex items-center justify-between px-3 py-2 text-[14px] text-white/85 hover:bg-white/10 transition"
                          >
                            <span>{group.label}</span>
                            {hasDropdown && (
                              <ChevronDown
                              ...
                              <div className="p-2 grid gap-1">
                                {group.items!.map((it) => (
                                  <a
                                    ...
                                    href={it.href}
                                    onClick={() => setMobileOpen(false)}
                                    className="rounded-xl px-3 py-2 text-[14px] text-white/80 hover:text-white hover:bg-white/10 transition"
                                  >
                                    {it.label}
                                  </a>
                                ))}
                    ...
                    {currentUser ? (
                      <div className="grid gap-2">
                        <button
                          ...
                          className="flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-[14px] bg-white/10 hover:bg-white/15 transition"
                        >
                          <User className="h-4 w-4" />
                          My Account
                        </button>
                        ...
                        {isAdmin && (
                          <button
                            ...
                            className="flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-[14px] bg-emerald-400/20 text-emerald-200 hover:bg-emerald-400/30 transition"
                          >
                            Admin Dashboard
                          </button>
                        )}
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-[14px] bg-red-500/90 text-black hover:bg-red-400 transition"
                        >
                          <LogOut className="h-4 w-4" />
                          Logout
                        </button>
